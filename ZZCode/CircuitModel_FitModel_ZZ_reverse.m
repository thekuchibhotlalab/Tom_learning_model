function [Parameters_fitted, fval] = CircuitModel_FitModel_ZZ_reverse (nUnit, ContextModulation, animal)
...
UpperBound = [Inf Inf Inf Inf 100 100 100 100 3 ];
LowerBound = [0 0 0 0 0 0 0 0 0 ];

%set the plausible upper and lower bounds for each parameter to increase
%accuracy
PlausibleLowerBound = [1e-4 1e-4 0.05 1.2 1e-3  1e-3  1e-3  1e-3 0];
PlausibleUpperBound = [5e-2 1e-2 0.4 4 2 2 2 2 2];

%select function being minimized
CostFun = @(x,animal,ContextModulation) CircuitModel_CostFun_ZZ_reverse(x,animal,ContextModulation,nUnit);
