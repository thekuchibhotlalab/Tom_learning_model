function [Cost] = CircuitModel_CostFun(params,animal,ContextModulation)
%--------------------------------------------------------------------------
...
W_I = [params(7) repmat(params(5),1,2)];         %   initital weights [S    S+    S-]
W_E = [params(8) repmat(params(6),1,2)];
...
for session = 1:courseness:max(reinforcedX);
    ...
    switch ContextModulation
        ...
        case 'inhibitory'
            ...
            prob_lick = (1 + exp(-(W_E(1) - c*W_I(1) + W_E(2) - c*W_I(2))./sig)).^(-1);
            reinforcedHit_Model(ctr) = prob_lick;
            
            %reinforced probability of licking, Foil tone
            prob_lick = (1 + exp(-(W_E(1) - c*W_I(1) + W_E(3) - c*W_I(3))./sig)).^(-1);
        ...
        case 'excitatory'
            ...
            %reinforced probability of licking, Target tone
            prob_lick = (1 + exp(-(c*W_E(1) - W_I(1) + c*W_E(2) - W_I(2))./sig)).^(-1);
            reinforcedHit_Model(ctr) = prob_lick;
            
            %reinforced probability of licking, Foil tone
            prob_lick = (1 + exp(-(c*W_E(1) - W_I(1) + c*W_E(3) - W_I(3))./sig)).^(-1);
            reinforcedFA_Model(ctr) = prob_lick;
        ...
        case 'threshold'
            ...
            %reinforced probability of licking, Target tone
            prob_lick = (1 + exp(-(W_E(1) - W_I(1) + W_E(2) - W_I(2) + c)./sig)).^(-1);
            reinforcedHit_Model(ctr) = prob_lick;
            
            %reinforced probability of licking, Foil tone
            prob_lick = (1 + exp(-(W_E(1) - W_I(1) + W_E(3) - W_I(3) + c)./sig)).^(-1);
            reinforcedFA_Model(ctr) = prob_lick;
            
            
        case 'gain'
            %reinforced probability of licking, Target tone
            prob_lick = (1 + exp(-(c*W_E(1) - c*W_I(1) + c*W_E(2) - c*W_I(2))./sig)).^(-1);
            reinforcedHit_Model(ctr) = prob_lick;
            
            %reinforced probability of licking, Foil tone
            prob_lick = (1 + exp(-(c*W_E(1) - c*W_I(1) + c*W_E(3) - c*W_I(3))./sig)).^(-1);
            reinforcedFA_Model(ctr) = prob_lick;        
    ...
    prob_lick = (1 + exp(-(W_E(1) - W_I(1) + W_E(2) - W_I(2))./sig)).^(-1);
    probeHit_Model(ctr) = prob_lick;
    
    %probe probability of licking, Foil Tone
    prob_lick = (1 + exp(-(W_E(1) - W_I(1) + W_E(3) - W_I(3))./sig)).^(-1);
